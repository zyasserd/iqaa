...
function parseIqaaString(str) {
    /*
    * "2/4:DT|DT" => { key: [2, 4], count: 4, unit: 8, rhythm: "DTDT", barIndices: [1] }
    * "10/8:Dss|Ts|DD|Tss" => { key: [10, 8], count: 10, unit: 8, rhythm: "DssTsDDTss", barIndices: [2, 4, 6] }
    */
    ...
    let rhythm = _parts[1].replaceAll('|', '');
    let count = rhythm.length;
    let unit = key[1] * count / key[0];
    let barIndices = [];
    for (let i = 0, j = 0; i < _parts[1].length; i++) {
        if (_parts[1][i] == '|') {
            barIndices.push(j);
        } else {
            j++;
    ...
    let ratio = count/key[0]; // == unit/key[1]

    return {key, count, unit, ratio, rhythm, barIndices};
}
...
class Notation {
    ...
    _iqaaToNotes() {
        ...
        for (let [i, c] of this.iqaaObj.rhythm.split('').entries()) {
        // for (let c of this.iqaaObj.rhythm) {
            textNotes.push(
                new Notation.VF.TextNote({
                    text: c,  duration: String(this.iqaaObj.unit),
            ...
            switch (c.toLowerCase()) {
                case 'd':
                    noteNotes.push(
                        new Notation.VF.StaveNote({
                            clef: "percussion", keys: ["b/4"], duration: String(this.iqaaObj.unit),
                ...
                case 's':
                    noteNotes.push(
                        new Notation.VF.StaveNote({
                            clef: "percussion", keys: ["b/4"], duration: `${this.iqaaObj.unit}r`
                        }).setAttribute('id', `N${i}`)
                    );
                    break;
                default:
    ...
    drawDividers(drawAllDividers=false) {
        ...
        for (let i = 0; i < this.notes.length; i++) {
            if (drawAllDividers || this.iqaaObj.barIndices.includes(i)) {
                this.drawLineX(
